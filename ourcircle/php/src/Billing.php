...
/** Stripe Checkout for Family Shield Pro. Not InPmnt. */
final class Billing
{
    public const PLANS = [
        'monthly' => ['name' => 'Family monthly', 'amount_label' => '$14.99/month', 'env_price' => 'STRIPE_PRICE_MONTHLY'],
        'yearly' => ['name' => 'Family yearly', 'amount_label' => '$119.99/year', 'env_price' => 'STRIPE_PRICE_YEARLY'],
    ];
    ...
    public static function publicBase(): string
    {
        ...
        return rtrim(Env::get('OURCIRCLE_SITE_URL', 'https://familyshieldpro.com'), '/');
    }
    ...
    public static function api(string $method, string $path, array $params = []): array
    {
        ...
        if (!function_exists('curl_init')) {
            throw new RuntimeException('PHP curl is required for Stripe. Enable it in hPanel → PHP Configuration.');
        }
    ...
    public static function createCheckout(array $args): array
    {
        ...
        $hid = (string) $args['household_id'];
        $uid = (string) $args['user_id'];
    ...
    {
        $q = '?expand[]=subscription&expand[]=subscription.items.data.price';
        return self::api('GET', '/v1/checkout/sessions/' . rawurlencode($sessionId) . $q);
    }
