...
_FAQ: list[tuple[tuple[str, ...], str]] = [
    ...
    (
        ...
        "Circle → Invite someone. We email a tap-to-open join link (the https://…/join/… URL is clickable, plus a Join button) if mail is set up (SMTP in .env), "
    ...
    (
        ...
        "A circle is up to five people in one household. Invite them from Check (home) with just an email, or from Circle. "
        "Circle status is Invited → Invite sent → Invite Accepted → User Accesses the Circle. "
        "Anyone in the circle can look at a check. Tap “Please call me before I pay” or “Send to family circle” to email "
