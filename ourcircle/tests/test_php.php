...
check(str_contains($circlePhp, 'Name or mobile'), 'circle invite optional extras');
...
check(str_contains($appPhp, 'lookEmailHtml'), 'look html email helper');
check(str_contains($appPhp, 'emailAndTextCircle'), 'circle notify helper');
...
check(str_contains($appPhp, 'join_invalid'), 'expired join stays public');
require_once $root . '/php/src/App.php';
$joinSample = 'https://sandbox.familyshieldpro.com/join/QxRZWCTaMjg49BbEt3zWvA';
$inviteHtml = App::inviteEmailHtml($joinSample);
check(str_contains($inviteHtml, '<a href="' . $joinSample . '"'), 'invite html url is an href');
check(str_contains($inviteHtml, '>' . $joinSample . '</a>'), 'invite html url is link text');
check(str_contains($inviteHtml, 'Join this family circle'), 'invite html join button text');
$inviteBody = App::inviteEmailBody($joinSample);
check(str_contains($inviteBody, '<' . $joinSample . '>'), 'invite text url in angle brackets');
check(str_contains($inviteBody, 'Open this link to join'), 'invite text open-this-link copy');
$joinInvalid = file_get_contents($root . '/php/views/join_invalid.php') ?: '';
check(str_contains($joinInvalid, 'This invite is not available'), 'join invalid page');
...
check(str_contains($mailPhp, 'quoted-printable'), 'smtp quoted-printable');
check(!str_contains($mailPhp, 'multipart/alternative'), 'smtp html-only not multipart');
check(!str_contains($mailPhp, 'Content-Transfer-Encoding: 8bit'), 'smtp not 8bit');
require_once $root . '/php/src/Http.php';
...
check(Product::version() === '1.2.26', 'product version 1.2.26');
...
check(str_contains($checkPhp, 'Send to family circle'), 'check send-to-circle button');
...
check(str_contains($homePhp, 'Invite by email'), 'home invite by email');
check(str_contains($homePhp, 'details class="more"'), 'home extra fields folded');
